...
const Animations = ({ player }: props) => {
    ...
    useEffect(() => { setTimeout(() => setMiddle(null), 300) }, [middle]);
    useEffect(() => { setTimeout(() => setLeft(null), 300) }, [left]);
    useEffect(() => { setTimeout(() => setRight(null), 300) }, [right]);
    useEffect(() => { setTimeout(() => setTop(null), 300) }, [top]);

    return (
        <div className="h-full w-full flex flex-col justify-start items-center">
            <div className="h-1/4 w-full flex justify-center items-end">
                <div className="h-14 aspect-video rounded-md flex justify-center items-center shadow-md lgt:hidden">
                    {top}
                </div>
            </div>
